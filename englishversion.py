...
def save_as_gif():
    ...
    progress_win.title("Rendering GIF")
    tk.Label(progress_win, text=f"Rendering {frames_count} frames...").pack(pady=5)
...
# ------------------------
root = tk.Tk()
root.title("Paper Background Animator (GIF Export)")

upload_frame = tk.Frame(root)
upload_frame.pack(pady=8)

upload_user_btn = tk.Button(upload_frame, text="Upload Image (PNG w/ transparency)", command=upload_user_image)
upload_user_btn.pack(side=tk.LEFT, padx=5)

upload_paper_btn = tk.Button(upload_frame, text="Upload Paper Texture", command=upload_paper_image)
upload_paper_btn.pack(side=tk.LEFT, padx=5)

upload_foreground_btn = tk.Button(upload_frame, text="Upload Foreground Wrinkle (optional)", command=upload_foreground_image)
...
tk.Label(slider_frame, text="Jaggedness:").grid(row=0, column=0, padx=6, sticky='e')
...
tk.Label(slider_frame, text="Paper Expansion:").grid(row=1, column=0, padx=6, sticky='e')
...
tk.Label(slider_frame, text="Wiggle Amount:").grid(row=2, column=0, padx=6, sticky='e')
...
tk.Label(slider_frame, text="GIF Frames:").grid(row=3, column=0, padx=6, sticky='e')
...
# Max Tilt
tk.Label(slider_frame, text="Max Tilt (°):").grid(row=4, column=0, padx=5, sticky='e')
...
# Animation Speed (FPS)
tk.Label(slider_frame, text="Animation Speed (FPS):").grid(row=5, column=0, padx=5, sticky='e')
...
tk.Label(slider_frame, text="Animation Speed (ms/frame):").grid(row=6, column=0, padx=6, sticky='e')
...
preview_label = tk.Label(root, text="Preview will appear here", bg="gray90")
preview_label.pack(pady=10)

anim_controls = tk.Frame(root)
anim_controls.pack(pady=6)

start_btn = tk.Button(anim_controls, text="Start Animation", command=start_animation)
start_btn.pack(side=tk.LEFT, padx=6)

stop_btn = tk.Button(anim_controls, text="Stop Animation", command=stop_animation)
stop_btn.pack(side=tk.LEFT, padx=6)

save_gif_btn = tk.Button(anim_controls, text="Save as GIF", command=save_as_gif)
save_gif_btn.pack(side=tk.LEFT, padx=6)

save_btn = tk.Button(root, text="Save Current Result (PNG)", command=lambda: save_combined())
save_btn.pack(pady=8)

def save_combined():
    global combined_img
    if combined_img is None:
        messagebox.showwarning("No image", "No combined image to save!")
        return
    save_path = filedialog.asksaveasfilename(defaultextension=".png",
                                             filetypes=[("PNG Image", "*.png")])
    if save_path:
        combined_img.save(save_path)
        messagebox.showinfo("Saved", f"Saved combined image as {save_path}")

instructions = tk.Label(root, text="1) Upload PNG with transparency (foreground)\n" +
                                  "2) Upload a paper texture (big, for shifting)\n" +
                                  "3) (Optional) Upload a transparent wrinkle overlay\n" +
                                  "4) Adjust Jaggedness / Expansion to control torn border\n" +
                                  "5) Use Wiggle & Speed for animation; click Start to preview\n" +
                                  "6) Click Save as GIF to export animation",
                       justify=tk.CENTER)
